...
class WinstonDataFormatter_v2:
    ...
    def format_data(self):
        ...
        for predicate_name, row in self.df_labels.iterrows():
            df_tmp = self.df_data.copy()
            df_tmp.loc[:, 'label'] = 'hello'

            for animal_name in self.target_animal_names:
                condition = df_tmp.loc[:, 'animal'] == animal_name
                df_tmp.loc[condition, 'label'] = row[animal_name]

            df_tmp = df_tmp.drop(['animal'], axis=1)
